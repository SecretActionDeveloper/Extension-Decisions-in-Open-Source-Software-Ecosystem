...
#   ###################################################################################
  action_names = list(df["Action_Name"])
...
#   ###################################################################################
  ...
  for i in range(len(df_atomic_Actions.index)):
      action = df_atomic_Actions["Action_Name"][i]
  ...
  for i in range(len(df_degree_2.index)):
      action = df_degree_2["Action_Name"][i]
  ...
  publisher_list = list(df["publisher"]); action_list =  list(df["Action_Name"])
  ...
  publisher_list = list(df["publisher"]); action_list_df =  list(df["Action_Name"])
  for n in nodes:
      if n in publisher_list:
        publishers.append(n)
...
## Make sure the dataset has all the necessary columns from the previous analysis (refer to the code above)
path = ".\CI_t0_processed.csv" 
save_vis = "Visualization_t0"
data_name = "t_0"
